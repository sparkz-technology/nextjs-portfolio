...
import { deleteGuestSignatureAction } from "@/app/dashboard/guestbook/action";
import { Dialog, DialogContent, DialogFooter, DialogHeader, DialogTitle } from "@/components/ui/dialog";

export const DeleteDialog: React.FC = () => {
  const { type, closeDialog, data } = useGuestSignatureDialog();
  ...
  const handleSubmit = async (_values: DeleteDialogValues, { setSubmitting }: FormikHelpers<DeleteDialogValues>) => {
    try {
      await deleteGuestSignatureAction(_values?.id);
      toast.info("Guest signature deleted successfully");
    } catch (err: unknown) {
  ...
  return (
    <Dialog open={type === "deleteGuestSignature"} onOpenChange={closeDialog}>
        ...
        {({ isSubmitting, submitForm }) => (
          <Form>
            <DialogContent className="sm:max-w-[425px] md:max-w-[600px] lg:max-w-[800px] w-full">
              ...
              <div className="py-6">
                <p className="text-lg font-semibold mb-2">Are you sure you want to delete this Guest signature?</p>
