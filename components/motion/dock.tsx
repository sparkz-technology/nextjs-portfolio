...
const Dock = React.forwardRef<HTMLDivElement, DockProps>(
  ({ className, children, magnification = DEFAULT_MAGNIFICATION, distance = DEFAULT_DISTANCE, ...props }, ref) => {
    const mousex = useMotionValue(Infinity); // Always create mousex here.

    const renderChildren = () => {
      return React.Children.map(children, (child: React.ReactNode) => {
        if (React.isValidElement(child)) {
          return React.cloneElement(child, {
            mousex, // Always pass down mousex here
            magnification,
            distance,
          } as DockIconProps);
        }
...
export interface DockIconProps {
  magnification?: number;
  distance?: number;
  mousex?: MotionValue<number>; // Ensure mousex is passed down unconditionally
  className?: string;
  children?: React.ReactNode;
